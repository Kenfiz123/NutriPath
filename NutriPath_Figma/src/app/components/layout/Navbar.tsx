...
export function Navbar({ isLanding = false }: NavbarProps) {
  ...
  const linkColor = isLanding ? "text-white/90 hover:text-white" : "text-gray-600 hover:text-green-600";
  const logoColor = isLanding ? "text-white" : "text-green-600";
  const buttonGhost = isLanding ? "text-white/90 hover:text-white hover:bg-white/10" : "text-gray-600 hover:text-green-700 hover:bg-green-50";
    ...
    <nav className={navBg}>
      <div className="max-w-[1440px] mx-auto px-8 h-16 flex items-center justify-between">
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all ${
                  isActive
                    ? isLanding ? "bg-white/20 text-white" : "bg-green-50 text-green-700"
                    : linkColor
                }`}
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {member ? (
            <>
              <button className={`relative p-2 rounded-full transition-all ${isLanding ? "text-white/80 hover:text-white hover:bg-white/10" : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"}`}>
                <Bell className="w-5 h-5" />
                <span className="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full" />
              </button>
              <Link to="/member" className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isLanding ? "bg-white/20 text-white" : "bg-green-100 text-green-700"}`} style={{ fontSize: "0.78rem", fontWeight: 800 }}>
                  {member.initials}
                </div>
                <span className={`${isLanding ? "text-white" : "text-gray-700"} hover:underline max-w-[130px] truncate`} style={{ fontSize: "0.875rem", fontWeight: 600 }}>
                  {member.name}
                </span>
              </Link>
              <button
        ...
        <button
          className={`md:hidden p-2 ${isLanding ? "text-white" : "text-gray-600"}`}
      ...
      {mobileOpen && (
        <div className={`md:hidden ${isLanding ? "bg-green-700/95" : "bg-white border-b border-gray-100"} px-6 py-4`}>
          {navLinks.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              to={href}
            ...
            {member ? (
              <>
                <Link
                  ...
                  onClick={() => setMobileOpen(false)}
                >
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center ${isLanding ? "bg-white/20" : "bg-green-100 text-green-700"}`} style={{ fontSize: "0.78rem", fontWeight: 800 }}>
                    {member.initials}
                  </span>
                  {member.name}
                </Link>
                <button
                  onClick={handleLogout}
                  className={`flex w-full items-center gap-2 py-2.5 ${isLanding ? "text-white" : "text-gray-700"}`}
                >
                  <LogOut className="w-4 h-4" />
                  Đăng xuất
                </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <Link
                  to="/login"
                  className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-2.5 ${isLanding ? "border-white/25 text-white" : "border-gray-200 text-gray-700"}`}
                  onClick={() => setMobileOpen(false)}
                >
                  <LogIn className="w-4 h-4" />
                  Đăng nhập
