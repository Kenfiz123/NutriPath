    ...
    <section className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-500/10">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-300">{t("hiện tại")}</p>
        </div>
        <div className="rounded-xl bg-blue-50 p-3 dark:bg-blue-500/10">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-300">{t("mục tiêu")}</p>
        </div>
      </div>
      <div className="mt-3 rounded-xl border border-gray-100 bg-gray-50 p-3 dark:border-slate-700 dark:bg-slate-900">
        <p className="text-sm font-extrabold text-gray-700 dark:text-slate-100">
          {t("Net hôm nay")}: {netCalories > 0 ? "+" : ""}
          {Math.round(netCalories).toLocaleString(locale)} kcal
        </p>
        <p className="mt-1 text-xs leading-5 text-gray-500 dark:text-slate-300">
          {t("Calo in {caloriesIn} - mục tiêu {calorieTarget} - calo out {caloriesOut}. Nếu giữ nhịp này 7 ngày, cân nặng có thể {direction} khoảng {delta}kg.", {
            caloriesIn: caloriesIn.toLocaleString(locale),
            calorieTarget: calorieTarget.toLocaleString(locale),
            caloriesOut: caloriesOut.toLocaleString(locale),
            direction: t(weeklyWeightDelta >= 0 ? "tăng" : "giảm"),
            delta: Math.abs(weeklyWeightDelta).toFixed(2),
          })}
        </p>
      </div>
    </section>
